...
# 1_mera, 3_mera, 5_mera = 1st, 3rd, 5th forecast day
...
# R writes into aaek_g4o2_2[,3,...]
# Python index 2
var_slot_index = 2

# R loop is for(kkk in 1:1), so only go3 is processed
variables_to_process = ["go3"]
...
if "aekkkkk_ff1" not in rdata:
    raise KeyError(f"'aekkkkk_ff1' not found. Available objects: {list(rdata.keys())}")
...
# Shape equivalent to R after dim(...)=c(3, ndays):
# member x init_day
# ============================================================
...
for run_name in run_names:
    ...
    path_csv = os.path.join(output_base, f"{run_name}_EU_aek_g4e2_name_nc.csv")
...
for run_name in run_names:
    ...
    output_prefix = os.path.join(output_base, f"{run_name}_EU_DJF_2026")
    ...
    for day_idx in range(ndays):
        ...
        for member_idx, lead_day in enumerate(forecast_leads):
            ...
            for varname in variables_to_process:
                ...
                # Keep first 8 times
                da = da.isel({time_name: slice(0, n_times_per_day)})

                # Interpolate ALL levels and stations at once
                interp_da = da.interp(
                    {
                ...
                # Equivalent to:
                # if stat_level == lev_nc[iiii], store that level
                mask = levels[:, None] == stat_level[None, :]

                vals_masked = np.where(mask[None, :, :], vals, np.nan)

                # Collapse level dimension.
                # Assumes each station matches one model level.
                out = np.nanmax(vals_masked, axis=1)  # time, station
        ...
        # R updates ntime once per day after the 3 members
        ntime += n_times_per_day
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            "description": "EU DJF 2026 station-interpolated model data",
            "run_name": run_name,
            "processed_variables": ",".join(variables_to_process),
            "all_variables": ",".join(all_parameters_cifs),
